...
func _process(delta: float):
	if alfa < 1.0:
		...
		global_position += Vector2(0, -1).rotated(rotation) * delta * speed
...
func explode(laser_rotation: float):
	emit_signal("exploded", global_position, laser_rotation, size, points)
	queue_free()
